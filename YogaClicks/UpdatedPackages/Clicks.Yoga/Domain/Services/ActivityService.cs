    ...
    public class ActivityService : ServiceBase, IActivityService
    {
        ...
        public IList<ActivityAttendee> GetUpcomingAttendances(EntityReference actor)
        {
            return EntityContext.ActivityAttendees
                ...
                .Include(e => e.Activity.ActivityVenue.Venue)
        ...
        public List<Activity> GetEventsForYear(int promoterId, bool isTeacher, bool isVenue, bool isTto)
        {
            if (isTeacher)
            {
                return EntityContext.Activities
                       .Where(x => x.Teachers.Any(t => t.Teacher.Id == promoterId && t.Confirmed))
            ...
            {
                return EntityContext.Activities
                       .Where(x => x.ActivityVenue.Venue.Id == promoterId && x.ActivityVenue.Confirmed)
                       .Where(x => !x.Type.IsClass)
                       .Where(x => x.StartTime > DateTime.Now && x.StartTime.Year == DateTime.Now.Year)
                       .Where(x => x.Active == true)
                       .OrderBy(x => x.StartTime)
        ...
        // ADD function actually replaces the venue as you can't have more than 1 venue attached to an activity (well you can but it will not work)
        public void AddActivityVenue(int activityId, int venueId)
        {
            var activity = GetActivity(activityId);
            ...
            AddActivityVenue(activity, venue);
        }

        public void AddActivityVenue(Activity activity, Venue venue)
        {
            if (activity == null)
                throw new ArgumentNullException("activity");
            if (venue == null)
            ...
            {
                if (activity.ActivityVenue.Id == venue.Id) return;
            }

            var confirmed = venue.Owner == null || venue.Owner.Id == activity.Owner.Id;

            var association = new ActivityVenue
            {
                Activity = activity,
                Venue = venue,
                Confirmed = confirmed
            ...
            // Add new
            activity.ActivityVenue = association;

            if (!association.Confirmed && venue.Owner != null)
            {
                RequestService.Request("ActivityVenueAdded", venue.Owner, SecurityContext.CurrentProfile, activity, venue, association);
            }

        }

        // Should never be able to remove.
        //public void RemoveActivityVenue(Activity activity, Venue venue)
        //{
        //    if (activity == null)
        //        throw new ArgumentNullException("activity");
        //    if (venue == null)
        //        throw new ArgumentNullException("venue");
        //    if (!SecurityContext.CanUpdateAny(activity, venue))
        //        throw new PermissionDeniedException();

        //    var association = activity.ActivityVenue;

        //    if (association == null) return;

        //    EntityContext.ActivityVenue.Remove(association);
        //    activity.ActivityVenue = null;
        //}
        ...
        public void SetActivityVenue(Activity activity, Venue venue)
        {
            ...
            //Remove current before adding new
            EntityContext.ActivityVenue.Remove(activity.ActivityVenue);

            if ((activity.ActivityVenue == null || activity.ActivityVenue.Id != venue.Id) && venue.Owner != null && (!SecurityContext.CanUpdate(venue) || SecurityContext.IsSuperUser()))
            {
            ...
            if (activity.ActivityVenue != null)
            {
                activity.ActivityVenue.Venue = venue;
            }
            else
            {
                activity.ActivityVenue = new ActivityVenue()
                {
                    Venue = venue
                };
            }
        ...
        private IQueryable<Activity> DefaultSearchQuery(ActivitySearchCriteria criteria)
        {
            var query = EntityContext.Activities
                ...
                .Include(a => a.ActivityVenue.Venue)
            ...
            if (criteria.Coordinates != null && criteria.Radius.HasValue && criteria.Radius.Value > 0)
            {
                ...
                query = query.Where(a => a.ActivityVenue.Venue.Address.LocationGeography.Distance(geography) <= criteria.Radius);
            }

            if (criteria.VenueId.HasValue)
            {
                query = query.Where(a => a.ActivityVenue.Venue.Id == criteria.VenueId);

                if (criteria.ConfirmedActivityTeacher)
                {
                }
            }
            ...
            if (!string.IsNullOrEmpty(criteria.VenueName))
            {
                ...
                query = query.Where(a => venueParts.Any(s => a.ActivityVenue.Venue.Name.Contains(s)));
            }
            ...
            if (criteria.TeacherId.HasValue)
            {
                ...
                if (criteria.ConfirmedActivityTeacher)
                {
                    query = query.Where(a => a.Teachers.Any(s => s.Confirmed && s.Teacher.Id == criteria.TeacherId));
                }
            }
            ...
            switch (criteria.SortOrder)
            {
                ...
                case ActivitySortOrder.Distance:
                    ...
                    {
                        var geography = DbGeographyFunctions.PointToGeography(criteria.Coordinates);
                        query = query.OrderBy(a => a.ActivityVenue.Venue.Address.LocationGeography.Distance(geography));
                    }
                    else
                    {
                        query = query.OrderBy(a => a.StartTime);
