...
namespace Clicks.Yoga.Portal.Controllers
{
    public class VenuesController : YogaController
    {
        ...
        public ActionResult Display(int id)
        {
            ...
            var teacher = venue.Owner != null ? TeacherService.GetTeacherForUser(venue.Owner.Id) : null;
            var tto = venue.Owner != null ? TeacherTrainingService.GetOrganisationForUser(venue.Owner.Id) : null;

            return View(new VenueDisplayModel(
                venue,
                ProfileService.GetProfile(venue),
                ReviewService.GetReviewsBySubject(venue),
                TeacherService.CurrentUserHasTeacher(),teacher,tto));
        }
        ...
        [HttpPost]
        [YogaAuthorize]
        public ActionResult CreateStubbed(string name, string location, double latitude, double longitude, string emailAddress)
        {
            var venue = new Venue();

            if (!string.IsNullOrEmpty(emailAddress))
            {
                venue = VenueService.CreateStubbed(name, new Location { Name = name, Coordinates = new GeographicPoint(latitude, longitude) }, emailAddress);    
            }
            else
            {
                venue = VenueService.CreateStubbed(name, new Location { Name = name, Coordinates = new GeographicPoint(latitude, longitude) });    
            }
            
            WorkUnit.Commit();
            var invite = EntityService.InviteEntity(venue, SecurityContext.CurrentUser, emailAddress);
        ...
        [YogaAuthorize]
        public ActionResult UpdateConditions(int id)
        {
            var venue = VenueService.GetVenueForEditing(id);

            return View(new VenueConditionsUpdateModel(venue).PopulateMetadata(venue, MedicService));
        }

        [HttpPost]
        [YogaAuthorize]
        public ActionResult UpdateConditions(VenueConditionsUpdateModel model)
        {
            var venue = VenueService.GetVenueForEditing(model.VenueId);

            model.PopulateEntity(venue, MedicService);
            venue.ModificationTime = DateTime.Now;
        ...
        [HttpPost]
        [YogaAuthorize]
        public ActionResult ClaimStep1(VenueClaimStep1Model model, [Deserialize] VenueClaimModel claimModel)
        {
            ViewBag.ClaimModel = claimModel;
        ...
        [HttpPost]
        [YogaAuthorize]
        public ActionResult ClaimStep2(VenueClaimStep2Model model, [Deserialize] VenueClaimModel claimModel)
        {
            ViewBag.ClaimModel = claimModel;
            if (model.Back)
            {
        ...
        [HttpPost]
        [YogaAuthorize]
        public ActionResult ClaimStep3(VenueClaimStep3Model model, [Deserialize] VenueClaimModel claimModel)
        {
            ViewBag.ClaimModel = claimModel;
