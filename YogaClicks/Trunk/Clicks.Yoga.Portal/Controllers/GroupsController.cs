...
namespace Clicks.Yoga.Portal.Controllers
{
    public class GroupsController : YogaController
    {
        ...
        [HttpPost]
        [YogaAuthorize]
        public ActionResult CreateStep1(GroupCreateStep1Model model, [Deserialize] GroupCreateModel createModel)
        {
            ViewBag.CreateModel = createModel;
        ...
        [HttpPost]
        [YogaAuthorize]
        public ActionResult CreateStep2(GroupCreateStep2Model model, [Deserialize] GroupCreateModel createModel)
        {
            ViewBag.CreateModel = createModel;
            if (model.Back)
            {
                ModelState.Clear();
                return View("CreateStep1", createModel.Step1Model.PopulateMetadata(ProfileService));
            }
        ...
        [HttpPost]
        [YogaAuthorize]
        public ActionResult CreateStep3(GroupCreateStep3Model model, [Deserialize] GroupCreateModel createModel)
        {
            ViewBag.CreateModel = createModel;
            ...
            {
                ModelState.Clear();
                return View("CreateStep2", createModel.Step2Model.PopulateMetadata(ProfileService));
            }
            ...
            createModel.PopulateEntity(group, SecurityContext.CurrentProfile, SecurityContext, EntityService, FriendService);

            GroupService.CreateGroup(group);
            WorkUnit.Commit();

            return View("CloseModal", new CloseModalModel(Url.Action("Display", "Groups", new { group.Id }), "CreateGroup"));
        }
