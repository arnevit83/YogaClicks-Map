...
namespace Clicks.Yoga.Portal.Controllers
{
    public class TeacherTrainingOrganisationsController : YogaController
    {
        ...
        public ActionResult Display(int id)
        {
            ...
            try
            {
                var profile = ProfileService.GetProfile(organisation.Owner.Id);
                ...
                return View(new OrganisationDisplayModel(organisation, teacher, venue));
            }
            catch (Exception)
            {
                return View(new OrganisationDisplayModel(organisation, null, null));
            }
        }
        ...
        public ActionResult Professionals(int id)
        {
            ...
            var TtoFaculty = organisation.Faculty;
            ...
            return View(new OrganisationProfessionalsModel(organisation, teachersAssociated.Where(x => !x.Stubbed).Distinct().ToList(), TtoFaculty.Select(x => x.FacultyMember).Distinct().ToList()));
        }
        ...
        public ActionResult Update(int id)
        {
            ...
            return View(new OrganisationUpdateModel(organisation, AddressService, StyleService, MedicService));
        }
        ...
        public ActionResult Update(int id, OrganisationUpdateModel model)
        {
            ...
            {
                model.PopulateEntity(organisation, AddressService, WebsiteService, ImageService, StyleService, MedicService);
        ...
        [HttpPost]
        [YogaAuthorize]
        public ActionResult UpdateConditions(OrganisationConditionsUpdateModel model)
        {
            var organisation = TeacherTrainingService.GetOrganisationForEditing(model.EntityId);

            model.PopulateEntity(organisation, MedicService);
            organisation.ModificationTime = DateTime.Now;
            WorkUnit.Commit();
        ...
        {
            return View(new OrganisationCreatePlacementModel());
        }


        [HttpPost]
        [YogaAuthorize]
        public ActionResult CreatePlacement(int id, OrganisationCreatePlacementModel model)
        {
            if (!ModelState.IsValid)
                return View(model);
            ...
            var tto = TeacherTrainingService.GetOrganisation(id);

            if (teacher == null || tto == null || tto.Faculty.Any(t => t.FacultyMember.Id == teacher.Id))
                return View("CloseModal");

            if (!SecurityContext.CurrentUser.CanUpdate(teacher) && !SecurityContext.CurrentUser.CanUpdate(tto))
                return View("CloseModal");

            TeacherTrainingService.AddFaculty(tto, teacher);
            WorkUnit.Commit();
        ...
        [HttpPost]
        [YogaAuthorize]
        public ActionResult RemovePlacement(int teacherId, int ttoId)
        {
            var tto = TeacherTrainingService.GetOrganisation(ttoId);
            var teacher = TeacherService.GetTeacher(teacherId);

            if (teacher == null || tto == null)
                return Json(false);

            if (!SecurityContext.CurrentUser.CanUpdate(teacher) && !SecurityContext.CurrentUser.CanUpdate(tto))
                return Json(false);

            var placement = teacher.FacultyMember.FirstOrDefault(p => p.Faculty.Id == tto.Id);

            if (placement == null)
                return Json(false);

            TeacherTrainingService.RemoveFaculty(tto, teacher);
            WorkUnit.Commit();
